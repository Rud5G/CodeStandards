...
class Rudger_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
...
//            }
    ...
    /**
     * Author tag must be 'Rudger Pty Ltd <mysource4@squiz.net>'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processAuthors($errorPos)
    {
        ...
        if (empty($authors) === false) {
            ...
            if (empty($content) === true) {
                ...
                $this->currentFile->addError($error, $errorPos);
            } else if ($content !== 'Rudger Pty Ltd <mysource4@squiz.net>') {
                $error = 'Expected "Rudger Pty Ltd <mysource4@squiz.net>" for author tag';
                $this->currentFile->addError($error, $errorPos);
            }
        }
    ...
    /**
     * Copyright tag must be in the form '2006-YYYY Rudger Pty Ltd (ABN 77 084 670 600)'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processCopyrights($errorPos)
    {
        ...
        if ($copyright !== null) {
            ...
            } else if (preg_match('/^([0-9]{4})-([0-9]{4})? (Rudger Pty Ltd \(ABN 77 084 670 600\))$/', $content) === 0) {
                $error = 'Expected "2006-2007 Rudger Pty Ltd (ABN 77 084 670 600)" for copyright declaration';
                $this->currentFile->addError($error, $errorPos);
            }
        }
    ...
    /**
     * License tag must be 'http://matrix.squiz.net/licence Rudger.Net Open Source Licence'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
        ...
        if ($license !== null) {
            ...
            if (empty($url) === true && empty($content) === true) {
                ...
                if (empty($content) === true) {
                    ...
                    $this->currentFile->addError($error, $errorPos);
                } else if ($content !== 'Rudger.Net Open Source Licence') {
                    $error = 'Expected "Rudger.Net Open Source Licence" for license name';
                    $this->currentFile->addError($error, $errorPos);
                }
            }//end if
        }//end if
